...
public class PageModel {
	...
	private int getCurrentPageFromRequestParameter(
		...
		if (currentPageParameter == null || currentPageParameter.equals(""))
			...
			try {
				...
				ControlerRedirectErrorException exception = new ControlerRedirectErrorException();
				exception
						.setExceptionMessage("Can not parse \"page\" parameter from request");
				throw exception;
			}
			if ((currentPageInteger - 1) * this.showElements.getIntegerValue() > this.numberOfElements)
				throw new ControlerErrorException("",
						"Bad page parameter, because there are "
								+ this.numberOfElements
								+ " but current page is " + currentPageInteger
								+ " with showElements "
	...
	private PageShowElements getShowElementsFromRequestParameter(
		...
		if (showElementsParameter == null || showElementsParameter == "")
			...
			try {
				...
				ControlerRedirectErrorException exception = new ControlerRedirectErrorException();
				exception
						.setExceptionMessage("Can not parse \"showElements\" parameter from request");
				throw exception;
			}
	 ...
	 */
	public void setShowElements(String newSize) throws NumberFormatException,
			ControlerErrorException {
